...
void mapSystematic()
{
	...
	TFile *input_file_data = new TFile(Form("output_dev/raw_results/c87/FF_data_out_histo_PbPb_5p02_r001.root"));
	cout << "Making map stat root file in output_dev/unfold" << endl;
	name = Form("./output_dev/unfold/UE_MapSystematic.root");
		...
		for (int i_dR = 0; i_dR < N_dR; i_dR++)
		{
			...
			for (int i_trk = 2; i_trk <= 8; i_trk++)
			{
				...
				for (int i_jet = 7; i_jet <= 12; i_jet++)
				{
					...
					h_sys_multiplier->SetBinContent(i_trk, i_jet,f_gaus->GetParameter(2)+1); //par2 is sigma, +1 to get relative error using width in terms of percentage
					output_file->cd();
