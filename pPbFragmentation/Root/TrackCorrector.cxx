...
float TrackCorrector::get_effcorr(float pt, float eta, int centrality, float uncert, float jet_pt, float jet_y, float R){
  ...
  if ( pt > jet_pt ) pt = jet_pt; //For tracks with z > 1 used the z = 1 efficiency 
  ...
  if (!_eff_jety || R>0.4) eff = _tf1_eff[etabin][centrality]->Eval(pt);
  	...
  	if (pt > 1.29) eff = _g_eff_jept[ybin][centrality][jetpt_bins->FindBin(jet_pt)]->Eval(pt); // Protection to the evaluation bellow 1 GeV
  	else eff = _g_eff_jept[ybin][centrality][jetpt_bins->FindBin(jet_pt)]->Eval(1.29); // Protection to the evaluation bellow 1 GeV, i.e. get the first bin content
  }    
  //pt weight
