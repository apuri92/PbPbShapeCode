...
EL::StatusCode PbPbFFShape :: histInitialize ()
{
	...
	double ptJetBins[1000], etaJetBins[1000], phiJetBins[1000], ptTrkBins[1000], etaTrkBins[1000], phiTrkBins[1000], zBins[1000], zBinsFine[1000],d0z0Bins[1000], respBins[1000], finehitsBins[1000], dR_resBins[1000], PsiBins[1000], RunBins[50], etaTrkBinsWide[100], phiTrkBinsWide[50];
...
//		wk()->addOutput (h_jetpT_v_multiplicity.at(i));
	...
	if (_data_switch==1)
	{
		...
		ChPS_TM_UE_rN =  vector<vector<TH3D*> > (_ndRBins, vector<TH3D*>(_nCentbins));
...
//	h_UE_dNdEtadPhidpT =  vector<vector<vector<vector<TH3D*>>>> (ptJetBinsN, vector<vector<vector<TH3D*>>> (1, vector<vector<TH3D*>> (_nCentbins, vector<TH3D*>(_ndRBins))));
	...
	for (int j=0;j<_nCentbins;j++)
	{
		...
		temphist_2D = new TH2D(Form("TM_norm_jet_rN_cent%i",j),Form("TM_norm_jet_rN_cent%i",j),RunBinsN, RunBins, ptJetBinsN, ptJetBins);
		TM_norm_jet_rN.at(j) = temphist_2D;
		wk()->addOutput (TM_norm_jet_rN.at(j));
		...
		for (int i=0;i<_ndRBins;i++)
		{
			...
			if (_uncert_index == 42)
			{
				for (int i_sys = 0; i_sys < ChPS_MB_UE_sys.size(); i_sys++)
				{
					...
					ChPS_MB_UE_sys.at(i_sys).at(i).at(j)->Sumw2();
					wk()->addOutput (ChPS_MB_UE_sys.at(i_sys).at(i).at(j));
				}
			}
			...
			temphist_2D = new TH2D(Form("ChPS_circ_UE_dR%i_cent%i",i,j),Form("ChPS_circ_UE_dR%i_cent%i",i,j),ptTrkBinsN, ptTrkBins, ptJetBinsN, ptJetBins);
			ChPS_circ_UE.at(i).at(j) = temphist_2D;
			...
			wk()->addOutput (ChPS_circ_UE.at(i).at(j));
			...
			if (_data_switch==1)
			{
				...
				temphist_3D = new TH3D(Form("ChPS_TM_UE_rN_dR%i_cent%i",i,j),Form("ChPS_TM_UE_rN_dR%i_cent%i",i,j), RunBinsN, RunBins, ptTrkBinsN, ptTrkBins, ptJetBinsN, ptJetBins);
				ChPS_TM_UE_rN.at(i).at(j) = temphist_3D;
