...
void combine_eff_dr(vector<TFile*>& theFiles, vector<double> w)
{
	...
	for (int i_cent_cuts = 0; i_cent_cuts < n_cent_cuts; i_cent_cuts++)
	{
		...
		for (int i_eta_cuts = 0; i_eta_cuts < n_trk_eta_bins_new; i_eta_cuts++)
		{
			...
			for (int i_dR = 0; i_dR < n_dR; i_dR++)
			{
				...
				{
					name = Form("total_3d_c%i_e%i_r%i_f%i", i_cent_cuts, i_eta_cuts, i_dR, i_files);
...
//					name = Form("entries_3d_c%i_e%i_r%i_f%i", i_cent_cuts, i_eta_cuts, i_dR, i_files);
...
//					h_entries_3d->SetName(name.c_str());
					...
					name = Form("matched_3d_c%i_e%i_r%i_f%i", i_cent_cuts, i_eta_cuts, i_dR, i_files);
...
//					h_entries_1d->SetName(name.c_str());
					...
					if (i_files == 0)
					{
						name = Form("h_efficiency_c%i_e%i_dR%i",i_cent_cuts, i_eta_cuts, i_dR);
						h_efficiency.at(i_cent_cuts).at(i_eta_cuts).at(i_dR) = (TH1*)h_matched_1d->Clone(name.c_str());
						h_efficiency.at(i_cent_cuts).at(i_eta_cuts).at(i_dR)->Reset();
					}

					name = Form("ratio_1d_c%i_e%i_dR%i_f%i",i_cent_cuts, i_eta_cuts, i_dR, i_files);
...
//						h_entries_1d->Draw("same hist text");
						...
						name = Form("%s, %4.2f < eta < %4.2f, %2.2f < r < %2.2f, JZ%i", num_to_cent(centrality_scheme, i_cent_cuts).c_str(), eta_lo, eta_hi, dr_lo, dr_hi, i_files+1);
						ltx->SetTextAlign(11);
						...
						if (i_eta_cuts == 0 && i_cent_cuts == 0 && i_dR == 0 && i_files == 0) name = "(";
						else if (i_eta_cuts == n_trk_eta_bins_new - 1  && i_cent_cuts == n_cent_cuts - 1 && i_dR == n_dR-1, i_files == nFiles - 1) name = ")";
						else name = "";
						canvas1->Print(Form("output_pdf/raw_eff_cent_trketa_dr_%s_%s.pdf%s", dataset_type.c_str(), tracking_cut.c_str(), name.c_str()),Form("Title: eff_c%i_e%i_dr%i_f%i", i_cent_cuts,i_eta_cuts, i_dR, i_files));
						canvas1->Clear();
					}
...
//						if (entries < 5) continue;
				...
				for (int i_bin = 0; i_bin < h_efficiency.at(i_cent_cuts).at(i_eta_cuts).at(i_dR)->GetNbinsX(); i_bin++)
				{
					...
					if (efficiency > 1.)
					{
						cout << Form("e>1 e:%f - cent:%i	eta:%i	bin:%f - %f",efficiency, i_cent_cuts, i_eta_cuts, h_efficiency.at(i_cent_cuts).at(i_eta_cuts).at(i_dR)->GetXaxis()->GetBinLowEdge(i_bin+1), h_efficiency.at(i_cent_cuts).at(i_eta_cuts).at(i_dR)->GetXaxis()->GetBinUpEdge(i_bin+1)) << endl;
					}

					h_efficiency.at(i_cent_cuts).at(i_eta_cuts).at(i_dR)->SetBinContent(i_bin+1, efficiency);
					h_efficiency.at(i_cent_cuts).at(i_eta_cuts).at(i_dR)->SetBinError(i_bin+1, efficiency_lo);
				}
				...
				g_efficiency.at(i_cent_cuts).at(i_eta_cuts).at(i_dR) = new TGraphAsymmErrors(h_efficiency.at(i_cent_cuts).at(i_eta_cuts).at(i_dR));
				name = Form("Efficiency: %s, %4.2f < #eta < %4.2f, %2.2f < r < %2.2f",centrality.c_str(), eta_lo, eta_hi, dr_lo, dr_hi);
				g_efficiency.at(i_cent_cuts).at(i_eta_cuts).at(i_dR)->SetTitle(name.c_str());

				name = Form("graph_eff_eta%i_cent%i_dr%i",i_eta_cuts,i_cent_cuts, i_dR);
				g_efficiency.at(i_cent_cuts).at(i_eta_cuts).at(i_dR)->SetName(name.c_str());
				g_efficiency.at(i_cent_cuts).at(i_eta_cuts).at(i_dR)->Write(name.c_str());
				...
				h_efficiency.at(i_cent_cuts).at(i_eta_cuts).at(i_dR)->SetName(name.c_str());
				h_efficiency.at(i_cent_cuts).at(i_eta_cuts).at(i_dR)->Write(name.c_str());
			}
		}
	}
