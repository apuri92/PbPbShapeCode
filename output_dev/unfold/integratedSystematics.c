...
void integratedSystematics(string config_file = "sys_config.cfg")
{
	...
	std::string integ_type = "lowpt_integ"; integ_type = m_config->GetValue("integ_type", integ_type.c_str());
	std::string mode = "ChPS"; mode = m_config->GetValue("mode", mode.c_str());
	...
	TFile* output_file = new TFile(Form("compiledCode/root/final_%s_%s_sys_%s%s.root", mode.c_str(), integ_type.c_str(), did.c_str(), dataset_type.c_str()), "recreate");

	TFile *f_pbpb, *f_pp;

	if (dataset_type.compare("") == 0)
	{
		f_pbpb = new TFile(Form("compiledCode/root/final_ChPS_%s_sys_data_PbPb.root", integ_type.c_str()));
		f_pp = new TFile(Form("compiledCode/root/final_ChPS_%s_sys_data_pp.root", integ_type.c_str()));
	}
	...
	vector<vector<TH1*>> h_nom (n_cent_cuts, vector<TH1*> (N_jetpt));
	vector<vector<TH1*>> h_total_sys_p (n_cent_cuts, vector<TH1*> (N_jetpt));
	vector<vector<TH1*>> h_total_sys_n (n_cent_cuts, vector<TH1*> (N_jetpt));


	//individual systematics
	vector<vector<vector<TH1*>>> h_sys (sys_names.size(), vector<vector<TH1*>> (n_cent_cuts, vector<TH1*> (N_jetpt)));
	vector<vector<vector<TH1*>>> h_sys_p (sys_names.size(), vector<vector<TH1*>> (n_cent_cuts, vector<TH1*> (N_jetpt)));
	vector<vector<vector<TH1*>>> h_sys_n (sys_names.size(), vector<vector<TH1*>> (n_cent_cuts, vector<TH1*> (N_jetpt)));

	//combined systematis
	vector<vector<vector<TH1*>>> h_comb_sys_p (combined_sys_names.size(), vector<vector<TH1*>> (n_cent_cuts, vector<TH1*> (N_jetpt)));
	vector<vector<vector<TH1*>>> h_comb_sys_n (combined_sys_names.size(), vector<vector<TH1*>> (n_cent_cuts, vector<TH1*> (N_jetpt)));
	...
	for (int i_sys = 0; i_sys < sys_names.size(); i_sys++)
	{
		...
		TFile *f_tmp = new TFile(name.c_str());
		...
		for (int i_jet = jet_pt_start; i_jet < jet_pt_end; i_jet++)
		{
			for (int i_cent = 0; i_cent < n_cent_cuts; i_cent++)
			{
				...
				name = Form("h_%s%s_%s_final_indR_cent%i_jetpt%i",integ_type.c_str(), dataset_type.c_str(), mode.c_str(), i_cent, i_jet);

				if (i_sys == 0)
				{
					h_nom[i_cent][i_jet] = (TH1*)nom_file->Get(name.c_str());
					h_nom[i_cent][i_jet]->SetName(Form("%s_nom", name.c_str()));
				}
				h_sys[i_sys][i_cent][i_jet] = (TH1*)sys_files[i_sys]->Get(name.c_str());
				h_sys[i_sys][i_cent][i_jet]->SetName(Form("%s_%s", name.c_str(), sys_names[i_sys].c_str()));
				h_sys[i_sys][i_cent][i_jet]->Add(h_nom[i_cent][i_jet], -1);
				h_sys[i_sys][i_cent][i_jet]->Divide(h_nom[i_cent][i_jet]);

				//					cout << Form("%i_%i_%i %i ", i_cent, i_jet, i_sys);
				//					cout << Form("%s --> x", sys_names[i_sys].c_str()) << endl;

				name = Form("h_%s_final_indR_cent%i_jetpt%i",mode.c_str(), i_cent, i_jet);
				h_sys_p[i_sys][i_cent][i_jet] = (TH1*)h_sys[i_sys][i_cent][i_jet]->Clone(Form("%s_%s_p", name.c_str(), sys_names[i_sys].c_str()) );
				h_sys_p[i_sys][i_cent][i_jet]->Reset();

				h_sys_n[i_sys][i_cent][i_jet] = (TH1*)h_sys[i_sys][i_cent][i_jet]->Clone(Form("%s_%s_n", name.c_str(), sys_names[i_sys].c_str()) );
				h_sys_n[i_sys][i_cent][i_jet]->Reset();

				//setup combined and total histo
				if (i_sys == 0)
				{
					name = Form("h_%s_final_indR_cent%i_jetpt%i_total_sys_pos", mode.c_str(), i_cent, i_jet);
					h_total_sys_p[i_cent][i_jet] = (TH1*)h_sys[i_sys][i_cent][i_jet]->Clone(name.c_str());
					h_total_sys_p[i_cent][i_jet]->Reset();

					name = Form("h_%s_final_indR_cent%i_jetpt%i_total_sys_neg", mode.c_str(), i_cent, i_jet);
					h_total_sys_n[i_cent][i_jet] = (TH1*)h_sys[i_sys][i_cent][i_jet]->Clone(name.c_str());
					h_total_sys_n[i_cent][i_jet]->Reset();

					for (int i_comb_sys = 0; i_comb_sys < combined_sys_names.size(); i_comb_sys++)
					{
						name = Form("h_%s_final_indR_cent%i_jetpt%i_comb_sys_pos",mode.c_str(), i_cent, i_jet);
						h_comb_sys_p[i_comb_sys][i_cent][i_jet] = (TH1*)h_sys[i_sys][i_cent][i_jet]->Clone(name.c_str());
						h_comb_sys_p[i_comb_sys][i_cent][i_jet]->Reset();

						name = Form("h_%s_final_indR_cent%i_jetpt%i_comb_sys_neg",mode.c_str(), i_cent, i_jet);
						h_comb_sys_n[i_comb_sys][i_cent][i_jet] = (TH1*)h_sys[i_sys][i_cent][i_jet]->Clone(name.c_str());
						h_comb_sys_n[i_comb_sys][i_cent][i_jet]->Reset();
					}
				}
				...
				{
					double tmp = h_sys[i_sys][i_cent][i_jet]->GetBinContent(i_dR);
					...
					{
						h_sys_p[i_sys][i_cent][i_jet]->SetBinContent(i_dR, fabs(tmp));
						h_sys_n[i_sys][i_cent][i_jet]->SetBinContent(i_dR, -fabs(tmp));
						h_sys_p[i_sys][i_cent][i_jet]->SetBinError(i_dR, 0.0000001);
						h_sys_n[i_sys][i_cent][i_jet]->SetBinError(i_dR, 0.0000001);
					}
					else //regular rules, setup pos and negative sides
					{
						if (tmp >= 0. ) h_sys_p[i_sys][i_cent][i_jet]->SetBinContent(i_dR, tmp);
						else h_sys_n[i_sys][i_cent][i_jet]->SetBinContent(i_dR, tmp);
						h_sys_p[i_sys][i_cent][i_jet]->SetBinError(i_dR, 0.0000001);
						h_sys_n[i_sys][i_cent][i_jet]->SetBinError(i_dR, 0.0000001);
					}
				...
				name = Form("h_%s_sys_cent%i_jetpt%i_%s", mode.c_str(), i_cent, i_jet, sys_names[i_sys].c_str());
				h_sys[i_sys][i_cent][i_jet]->Write(name.c_str());

				name = Form("h_%s_sys_cent%i_jetpt%i_%s_p", mode.c_str(), i_cent, i_jet, sys_names[i_sys].c_str());
				h_sys_p[i_sys][i_cent][i_jet]->Write(name.c_str());

				name = Form("h_%s_sys_cent%i_jetpt%i_%s_n",mode.c_str(), i_cent, i_jet, sys_names[i_sys].c_str());
				h_sys_n[i_sys][i_cent][i_jet]->Write(name.c_str());

				if (i_sys == 0)
				{
					name = Form("h_%s_final_indR_cent%i_jetpt%i",mode.c_str(), i_cent, i_jet);
					h_nom[i_cent][i_jet]->Write(name.c_str());
				}
				//				}
			}
		}
			...
			for (int i_comb_sys = 0; i_comb_sys < combined_sys_names.size(); i_comb_sys++)
			{
				for (int i_dR = 1; i_dR <= N_dR; i_dR++)
				{
					for (int i_sys = 0; i_sys < sys_names.size(); i_sys++)
					{
						...
						//							cout << Form("%i_%i_%i_%i %i_%i ", i_cent, i_jet, i_dR, i_sys, i_comb_sys);
							...
							)
						{
							...
							tmp = h_sys_p[i_sys][i_cent][i_jet]->GetBinContent(i_dR);
							h_comb_sys_p[i_comb_sys][i_cent][i_jet]->SetBinContent(i_dR, tmp);
							h_comb_sys_p[i_comb_sys][i_cent][i_jet]->SetBinError(i_dR, 0.0000001);

							tmp = h_sys_n[i_sys][i_cent][i_jet]->GetBinContent(i_dR);
							h_comb_sys_n[i_comb_sys][i_cent][i_jet]->SetBinContent(i_dR, tmp);
							h_comb_sys_n[i_comb_sys][i_cent][i_jet]->SetBinError(i_dR, 0.0000001);
						}
							...
							)
						{
							...
							tmp = pow(h_comb_sys_p[i_comb_sys][i_cent][i_jet]->GetBinContent(i_dR),2);
							tmp = tmp + pow(h_sys_p[i_sys][i_cent][i_jet]->GetBinContent(i_dR),2);
							h_comb_sys_p[i_comb_sys][i_cent][i_jet]->SetBinContent(i_dR, sqrt(tmp) );
							h_comb_sys_p[i_comb_sys][i_cent][i_jet]->SetBinError(i_dR, 0.0000001);

							tmp = pow(h_comb_sys_n[i_comb_sys][i_cent][i_jet]->GetBinContent(i_dR),2);
							tmp = tmp + pow(h_sys_n[i_sys][i_cent][i_jet]->GetBinContent(i_dR),2);
							h_comb_sys_n[i_comb_sys][i_cent][i_jet]->SetBinContent(i_dR, -sqrt(tmp) );
							h_comb_sys_n[i_comb_sys][i_cent][i_jet]->SetBinError(i_dR, 0.0000001);
...

				{
					...
					name = Form("h_ChPS_final_indR_cent%i_jetpt%i",  i_cent, i_jet);
					TH1* h_pbpb_nom = (TH1*)f_pbpb->Get(name.c_str());
					h_pbpb_nom->SetName(Form("%s_pbpb", name.c_str()));

					name = Form("h_ChPS_final_indR_cent%i_jetpt%i",  6, i_jet);
					TH1* h_pp_nom = (TH1*)f_pp->Get(name.c_str());
					h_pp_nom->SetName(Form("%s_pp", name.c_str()));

					name = Form("h_ChPS_sys_cent%i_jetpt%i_%s_p",  i_cent, i_jet,combined_sys_names[i_comb_sys].c_str());
					TH1* h_pbpb_p = (TH1*)f_pbpb->Get(name.c_str());
					h_pbpb_p->SetName(Form("%s_pbpb", name.c_str()));

					name = Form("h_ChPS_sys_cent%i_jetpt%i_%s_p",  6, i_jet,combined_sys_names[i_comb_sys].c_str());
					TH1* h_pp_p = (TH1*)f_pp->Get(name.c_str());
					h_pp_p->SetName(Form("%s_pp", name.c_str()));

					name = Form("h_ChPS_sys_cent%i_jetpt%i_%s_n",  i_cent, i_jet,combined_sys_names[i_comb_sys].c_str());
					TH1* h_pbpb_n = (TH1*)f_pbpb->Get(name.c_str());
					h_pbpb_n->SetName(Form("%s_pbpb", name.c_str()));

					name = Form("h_ChPS_sys_cent%i_jetpt%i_%s_n",  6, i_jet,combined_sys_names[i_comb_sys].c_str());
					...
					if (mode == "RDpT")
					{
						...
						for (int i_dR = 1; i_dR <= N_dR; i_dR++)
						{
							...
							h_comb_sys_p[i_comb_sys][i_cent][i_jet]->SetBinContent(i_dR, dR_overR);
							h_comb_sys_p[i_comb_sys][i_cent][i_jet]->SetBinError(i_dR, 0.000001);

							dA_overA = h_pbpb_n->GetBinContent(i_dR);
							dB_overB = h_pp_n->GetBinContent(i_dR);
							dR_overR = sqrt( pow(dA_overA,2) + pow(dB_overB,2) );
							h_comb_sys_n[i_comb_sys][i_cent][i_jet]->SetBinContent(i_dR, -dR_overR);
							h_comb_sys_n[i_comb_sys][i_cent][i_jet]->SetBinError(i_dR, 0.000001);
						}
					}
				...
				//					{
				//						h_comb_sys_p[i_comb_sys][i_cent][i_jet]->Smooth(1,"");
				//						h_comb_sys_n[i_comb_sys][i_cent][i_jet]->Smooth(1,"");
				//					}

				name = Form("h_%s_sys_cent%i_jetpt%i_%s_p",mode.c_str(), i_cent, i_jet,combined_sys_names[i_comb_sys].c_str());
				h_comb_sys_p[i_comb_sys][i_cent][i_jet]->SetTitle(name.c_str());
				h_comb_sys_p[i_comb_sys][i_cent][i_jet]->Write(name.c_str());

				name = Form("h_%s_sys_cent%i_jetpt%i_%s_n",mode.c_str(), i_cent, i_jet,combined_sys_names[i_comb_sys].c_str());
				h_comb_sys_n[i_comb_sys][i_cent][i_jet]->SetTitle(name.c_str());
				h_comb_sys_n[i_comb_sys][i_cent][i_jet]->Write(name.c_str());
	...
	for (int i_jet = jet_pt_start; i_jet < jet_pt_end; i_jet++)
	{
		for (int i_cent = 0; i_cent < n_cent_cuts; i_cent++)
		{
			...
			for (int i_dR = 1; i_dR <= N_dR; i_dR++)
			{
				...
				for (int i_comb_sys = 0; i_comb_sys < combined_sys_names.size(); i_comb_sys++)
				{
					tmp_p = tmp_p+pow(h_comb_sys_p[i_comb_sys][i_cent][i_jet]->GetBinContent(i_dR),2);
					tmp_n = tmp_n+pow(h_comb_sys_n[i_comb_sys][i_cent][i_jet]->GetBinContent(i_dR),2);
				}
				h_total_sys_p[i_cent][i_jet]->SetBinContent(i_dR, sqrt(tmp_p) );
				h_total_sys_p[i_cent][i_jet]->SetBinError(i_dR, 0.0000001 );

				h_total_sys_n[i_cent][i_jet]->SetBinContent(i_dR, -sqrt(tmp_n) );
				h_total_sys_n[i_cent][i_jet]->SetBinError(i_dR, 0.0000001 );
			}

			name = Form("h_%s_sys_cent%i_jetpt%i_total_p",mode.c_str(), i_cent, i_jet);
			h_total_sys_p[i_cent][i_jet]->Write(name.c_str());

			name = Form("h_%s_sys_cent%i_jetpt%i_total_n",mode.c_str(), i_cent, i_jet);
			h_total_sys_n[i_cent][i_jet]->Write(name.c_str());
	...
	//					for (int i_trk = 0; i_trk < N_trkpt; i_trk++)
	//					{
	//						val = h_total_sys_p[i_cent][i_jet]->GetBinContent(i_dR+1);
	//						val_err = h_total_sys_p[i_cent][i_jet]->GetBinError(i_dR+1);
	...
	//
	//						val = h_total_sys_n[i_cent][i_jet]->GetBinContent(i_dR+1);
	//						val_err = h_total_sys_n[i_cent][i_jet]->GetBinError(i_dR+1);
	...
	for (int i_jet = jet_pt_start; i_jet < jet_pt_end; i_jet++)
	{
		...
		//		{
					string trk_label = Form("%1.1f < #it{p}_{T} < %1.1f GeV", 1., 4.);
		...
		for (int i_cent = 0; i_cent < n_cent_cuts; i_cent++)
		{
			...
			SetHStyle_smallify(h_total_sys_p[i_cent][i_jet],0, doSmall);
			SetHStyle_smallify(h_total_sys_n[i_cent][i_jet],0, doSmall);
			h_total_sys_p[i_cent][i_jet]->SetLineWidth(1);
			h_total_sys_n[i_cent][i_jet]->SetLineWidth(1);
			h_total_sys_p[i_cent][i_jet]->SetMarkerStyle(20);
			h_total_sys_n[i_cent][i_jet]->SetMarkerStyle(20);

			h_total_sys_p[i_cent][i_jet]->GetYaxis()->SetRangeUser(-0.8,0.8);
			h_total_sys_n[i_cent][i_jet]->GetYaxis()->SetRangeUser(-0.8,0.8);
			h_total_sys_p[i_cent][i_jet]->GetXaxis()->SetRangeUser(0, r_max_range);
			h_total_sys_n[i_cent][i_jet]->GetXaxis()->SetRangeUser(0, r_max_range);
			h_total_sys_p[i_cent][i_jet]->GetYaxis()->SetTitle(y_label.c_str());
			h_total_sys_n[i_cent][i_jet]->GetYaxis()->SetTitle(y_label.c_str());
			h_total_sys_p[i_cent][i_jet]->GetXaxis()->SetTitle(r_label.c_str());
			h_total_sys_n[i_cent][i_jet]->GetXaxis()->SetTitle(r_label.c_str());


			c_sys->cd(i_cent+1);
			//				h_total_sys_p[i_cent][i_jet]->GetYaxis()->SetRangeUser(-0.1, 0.1);
			h_total_sys_p[i_cent][i_jet]->Draw("p");
			h_total_sys_n[i_cent][i_jet]->Draw("p same");

			//				for (int i_sys = 0; i_sys < sys_names.size(); i_sys++)
			//				{
			//					SetHStyle_smallify(h_sys_p[i_sys][i_cent][i_jet],i_sys+1,1);
			//					SetHStyle_smallify(h_sys_n[i_sys][i_cent][i_jet],i_sys+1,1);
			//					h_sys_p[i_sys][i_cent][i_jet]->SetLineStyle(1);
			//					h_sys_n[i_sys][i_cent][i_jet]->SetLineStyle(1);
			//					h_sys_p[i_sys][i_cent][i_jet]->SetLineWidth(1);
			//					h_sys_n[i_sys][i_cent][i_jet]->SetLineWidth(1);
			//
			//					if (cent_first_pass) legend_sys->AddEntry(h_sys_p[i_sys][i_cent][i_jet],sys_i_name[sys_names[i_sys]].c_str(),"lp");
			//					h_sys_p[i_sys][i_cent][i_jet]->Draw("p same");
			//					h_sys_n[i_sys][i_cent][i_jet]->Draw("p same");
			//
			//				}
			for (int i_comb_sys = 0; i_comb_sys < combined_sys_names.size(); i_comb_sys++)
			{
				SetHStyle_smallify(h_comb_sys_p[i_comb_sys][i_cent][i_jet],i_comb_sys+1, doSmall);
				SetHStyle_smallify(h_comb_sys_n[i_comb_sys][i_cent][i_jet],i_comb_sys+1, doSmall);
				h_comb_sys_p[i_comb_sys][i_cent][i_jet]->SetLineWidth(1);
				h_comb_sys_n[i_comb_sys][i_cent][i_jet]->SetLineWidth(1);
				h_comb_sys_p[i_comb_sys][i_cent][i_jet]->SetLineStyle(2);
				h_comb_sys_n[i_comb_sys][i_cent][i_jet]->SetLineStyle(2);

				bool empty_hist_p = false;
				bool empty_hist_n = false;
				if (h_comb_sys_p[i_comb_sys][i_cent][i_jet]->GetMaximum() == 0 && h_comb_sys_p[i_comb_sys][i_cent][i_jet]->GetMinimum() == 0) empty_hist_p = true;
				if (h_comb_sys_n[i_comb_sys][i_cent][i_jet]->GetMaximum() == 0 && h_comb_sys_n[i_comb_sys][i_cent][i_jet]->GetMinimum() == 0) empty_hist_n = true;

				if (cent_first_pass) legend_sys->AddEntry(h_comb_sys_p[i_comb_sys][i_cent][i_jet],combined_sys_names[i_comb_sys].c_str(),"lp");

				//DO NOT SET RANGES ON COMB PLOTS. THIS MESSES UP THE DRAW SAME OPTION
				h_comb_sys_p[i_comb_sys][i_cent][i_jet]->GetXaxis()->SetRangeUser(0, r_max_range);
				h_comb_sys_n[i_comb_sys][i_cent][i_jet]->GetXaxis()->SetRangeUser(0, r_max_range);
				h_comb_sys_p[i_comb_sys][i_cent][i_jet]->GetYaxis()->SetTitle(y_label.c_str());
				h_comb_sys_n[i_comb_sys][i_cent][i_jet]->GetYaxis()->SetTitle(y_label.c_str());
				h_comb_sys_p[i_comb_sys][i_cent][i_jet]->GetXaxis()->SetTitle(r_label.c_str());
				h_comb_sys_n[i_comb_sys][i_cent][i_jet]->GetXaxis()->SetTitle(r_label.c_str());

				c_sys->cd(i_cent+1);
				if (!empty_hist_p) h_comb_sys_p[i_comb_sys][i_cent][i_jet]->Draw("p same");
				if (!empty_hist_n) h_comb_sys_n[i_comb_sys][i_cent][i_jet]->Draw("p same");
			}
			...
			if (cent_first_pass) legend_sys->AddEntry(h_total_sys_p[i_cent][i_jet],"Total","pl");
		...
		c_sys->Print(Form("compiledCode/systematics/Summary_%s_%s_dR_sys%s_error.pdf%s",integ_type.c_str(), mode.c_str(), dataset_type.c_str(), pdf_label.c_str()), Form("Title:jetpt%i", i_jet));
