...
void systematics_dev(string dataset_type, TFile *output_file)
{
	...
	TFile* nom_file = new TFile(Form("output_pdf_nominal/root/final_ChPS_%s_%s.root", did.c_str(), dataset_type.c_str()));
	TFile* output_file = new TFile(Form("output_pdf_nominal/root/final_ChPS_sys_%s_%s.root", did.c_str(), dataset_type.c_str()), "recreate");
...
//	sys_names.push_back("sys201"); //Fakes
	...
	{
		name = Form("output_pdf_%s/root/final_ChPS_%s%s.root", sys_names[i_sys].c_str(), did.c_str(), dataset_type.c_str());
		...
		for (int i_jet = jet_pt_start; i_jet < jet_pt_end; i_jet++)
		{
			for (int i_cent = 0; i_cent < n_cent_cuts; i_cent++)
			{
				...
				for (int i_trk = 0; i_trk < N_trkpt; i_trk++)
				{
					...
					if (i_sys == 0)
					{
						name = Form("h_ChPS_final_indR_trk%i_cent%i_jetpt%i_total_sys_pos",  i_trk, i_cent, i_jet);
						...
						name = Form("h_ChPS_final_indR_trk%i_cent%i_jetpt%i_total_sys_neg",  i_trk, i_cent, i_jet);
						...
						{
							name = Form("h_ChPS_final_indR_trk%i_cent%i_jetpt%i_comb_sys_pos", i_trk, i_cent, i_jet);
							...
							name = Form("h_ChPS_final_indR_trk%i_cent%i_jetpt%i_comb_sys_neg", i_trk, i_cent, i_jet);
					...
					for (int i_dR = 1; i_dR <= N_dR; i_dR++)
					{
						...
						if (sys_names[i_sys] == "sys101" || sys_names[i_sys] == "sys200" || sys_names[i_sys] == "sys201" || sys_names[i_sys] == "sys202")
						{
					...
					name = Form("h_ChPS_sys_trk%i_cent%i_jetpt%i_%s", i_trk, i_cent, i_jet, sys_names[i_sys].c_str());
					h_sys[i_sys][i_trk][i_cent][i_jet]->Write(name.c_str());

					name = Form("h_ChPS_sys_trk%i_cent%i_jetpt%i_%s_p", i_trk, i_cent, i_jet, sys_names[i_sys].c_str());
					h_sys_p[i_sys][i_trk][i_cent][i_jet]->Write(name.c_str());

					name = Form("h_ChPS_sys_trk%i_cent%i_jetpt%i_%s_n", i_trk, i_cent, i_jet, sys_names[i_sys].c_str());
	...
	string dptr_label = "#it{D} (#it{p}_{T}, #it{r})";
	string r_label = "#it{r}";
	...
	for (int i_jet = jet_pt_start; i_jet < jet_pt_end; i_jet++)
	{
		...
		for (int i_trk = trk_pt_start; i_trk < trk_pt_end; i_trk++)
		{
			...
			for (int i_cent = 0; i_cent < n_cent_cuts; i_cent++)
			{
				...
				h_total_sys_p[i_trk][i_cent][i_jet]->GetYaxis()->SetTitle(Form("#delta%s",dptr_label.c_str()));
				h_total_sys_n[i_trk][i_cent][i_jet]->GetYaxis()->SetTitle(Form("#delta%s",dptr_label.c_str()));
				...
				for (int i_comb_sys = 0; i_comb_sys < combined_sys_names.size(); i_comb_sys++)
				{
					...
					h_comb_sys_p[i_comb_sys][i_trk][i_cent][i_jet]->GetYaxis()->SetTitle(Form("#delta%s",dptr_label.c_str()));
					h_comb_sys_n[i_comb_sys][i_trk][i_cent][i_jet]->GetYaxis()->SetTitle(Form("#delta%s",dptr_label.c_str()));
			...
			if (dataset_type == "PbPb") ltx->DrawLatexNDC(0.2, 0.75, "Pb+Pb #sqrt{#font[12]{s_{NN}}} = 5.02 TeV, 0.49 nb^{-1}");
			if (dataset_type == "pp") ltx->DrawLatexNDC(0.2, 0.8, "#it{pp} #sqrt{#font[12]{s}} = 5.02 TeV, 25 pb^{-1}");

			string pdf_label = "";
			if (i_trk == trk_pt_start && i_jet == jet_pt_start) pdf_label = "(";
			if (i_trk == trk_pt_end-1 && i_jet == jet_pt_end-1) pdf_label = ")";
			c_sys->Print(Form("output_pdf_nominal/systematics/ChPS_dR_sys%s_error.pdf%s", dataset_type.c_str(), pdf_label.c_str()), Form("Title:trk%i_jetpt%i", i_trk, i_jet));
