...
void systematics(bool isMC = 0)
{
	...
	TFile* output_file = new TFile(Form("output_pdf_nominal/root/final_RDpT_sys_%sy.root", did.c_str()), "recreate");
	...
	vector<vector<vector<TH1*>>> h_ChPS_RDpT_indR_sys_Signpos (N_trkpt, vector<vector<TH1*>> (n_cent_cuts, vector<TH1*> (N_jetpt)));
	vector<vector<vector<TH1*>>> h_ChPS_RDpT_indR_sys_Signneg (N_trkpt, vector<vector<TH1*>> (n_cent_cuts, vector<TH1*> (N_jetpt)));
			...
			for (int i_cent = 0; i_cent < 6; i_cent++)
			{
				for (int i_trk = 0; i_trk < N_trkpt; i_trk++)
				{
					...
					if (i == 0)
					{
						...
						//Sign
						name = Form("h_RDpT_final_indR_trk%i_cent%i_jetpt%i", i_trk, i_cent, i_jet);
						h_ChPS_RDpT_indR_sys_Signpos[i_trk][i_cent][i_jet] = (TH1*)h_ChPS_RDpT_indR_sys[i][i_trk][i_cent][i_jet]->Clone(Form("%s_sys_Signpos", name.c_str()));
						h_ChPS_RDpT_indR_sys_Signpos[i_trk][i_cent][i_jet]->Reset();

						h_ChPS_RDpT_indR_sys_Signneg[i_trk][i_cent][i_jet] = (TH1*)h_ChPS_RDpT_indR_sys[i][i_trk][i_cent][i_jet]->Clone(Form("%s_sys_Signneg", name.c_str()));
						h_ChPS_RDpT_indR_sys_Signneg[i_trk][i_cent][i_jet]->Reset();
	...
	for (int i_jet = jet_pt_start; i_jet < jet_pt_end; i_jet++)
	{
		for (int i_cent = 0; i_cent < 6; i_cent++)
		{
			for (int i_trk = 0; i_trk < N_trkpt; i_trk++)
			{
				for (int i_dR = 1; i_dR <= N_dR; i_dR++)
				{
					for (int i = 0; i < sys_names.size(); i++)
					{
						...
						if (sys_names[i] == "sys101")
						{
							...
							if (tmp >= 0) h_ChPS_RDpT_indR_sys_Signpos[i_trk][i_cent][i_jet]->SetBinContent(i_dR, tmp);
							else h_ChPS_RDpT_indR_sys_Signneg[i_trk][i_cent][i_jet]->SetBinContent(i_dR, tmp);
						...
						if (sys_names[i] == "sys200")
						{
							...
							h_ChPS_RDpT_indR_sys_UEpos[i_trk][i_cent][i_jet]->SetBinContent(i_dR, fabs(tmp));
							h_ChPS_RDpT_indR_sys_UEneg[i_trk][i_cent][i_jet]->SetBinContent(i_dR, -fabs(tmp));
						}
					...
					tmp = tmp + pow(h_ChPS_RDpT_indR_sys_Signpos[i_trk][i_cent][i_jet]->GetBinContent(i_dR),2);
					...
					tmp = tmp + pow(h_ChPS_RDpT_indR_sys_Signneg[i_trk][i_cent][i_jet]->GetBinContent(i_dR),2);
				...
				h_ChPS_RDpT_indR_sys_Signpos[i_trk][i_cent][i_jet]->SetName(name.c_str());
				h_ChPS_RDpT_indR_sys_Signpos[i_trk][i_cent][i_jet]->SetTitle(name.c_str());
				h_ChPS_RDpT_indR_sys_Signpos[i_trk][i_cent][i_jet]->Write(name.c_str());

				name = Form("h_ChPS_RDpT_indR_trk%i_cent%i_jetpt%i_sys_Signneg", i_trk, i_cent, i_jet);
				h_ChPS_RDpT_indR_sys_Signneg[i_trk][i_cent][i_jet]->SetName(name.c_str());
				h_ChPS_RDpT_indR_sys_Signneg[i_trk][i_cent][i_jet]->SetTitle(name.c_str());
				h_ChPS_RDpT_indR_sys_Signneg[i_trk][i_cent][i_jet]->Write(name.c_str());
	...
	for (int i_jet = jet_pt_start; i_jet < jet_pt_end; i_jet++)
	{
		...
		for (int i_trk = trk_pt_start; i_trk < trk_pt_end; i_trk++)
		{
			...
			c_sys->Print(Form("output_pdf_nominal/RDpT_dR_sys_errory.pdf%s", pdf_label.c_str()), Form("Title:trk%i_jetpt%i", i_trk, i_jet));
