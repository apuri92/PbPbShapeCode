		...
		for (int i_cent = 0; i_cent < 6; i_cent++)
		{
			...
			for (int i_trk = 0; i_trk < N_trkpt; i_trk++)
			{
				...
				for (int i_dR = 0 ; i_dR < N_dR; i_dR++)
				{
					...
					r_width = 0.030; //h_RDpT_final_ratio_indR[i_trk][i_cent][i_jet]->GetBinWidth(i_dR+1)/4;
					...
					r_width = 0.040; //h_ChPS_PbPb_final_ratio_indR[i_trk][i_cent][i_jet]->GetBinWidth(i_dR+1);
					...
					if (i_cent == 0)
					{
						...
						r_width = 0.040; //h_ChPS_pp_final_ratio_indR[i_trk][6][i_jet]->GetBinWidth(i_dR+1);
...
//		legend->SetNColumns(1);
		...
		TLegend *legend = new TLegend(0.19, 0.57, 0.80, 0.75, "","brNDC");
		legend->SetTextFont(43);
		legend->SetBorderSize(0);
...
//					h_RDpT_final_ratio_indR[i_trk][i_cent][i_jet]->Draw(" same");
				...
				if (i_trk == trk_select1) ltx->DrawLatexNDC(0.65,0.91, Form("%1.1f-%1.1f", trkpT_binning->GetBinLowEdge(i_trk+1), trkpT_binning->GetBinUpEdge(i_trk+1)));
				if (i_trk == trk_select2) ltx->DrawLatexNDC(0.73,0.91, Form("%1.1f-%1.1f", trkpT_binning->GetBinLowEdge(i_trk+1), trkpT_binning->GetBinUpEdge(i_trk+1)));
...
//			canvas->Print(Form("output_pdf_nominal/conf/RDpT_final_ratio_dR_CONF_%s_trk_cent%i.root", did.c_str(), i_cent));
...
//					if (i_cent%2!=0) continue;
					...
					if (i_trk == 6 && jet_itr == 0)
					{
						cout << g_RDpT_final_sys_indR[i_trk][i_cent][i_jet]->GetName() << " ";
						cout << g_RDpT_final_sys_indR[i_trk][i_cent][i_jet]->GetFillColor() << endl;
					}
