...
void comp_ChPS()
{
	...
	for (int i_jet = jet_pt_start; i_jet < jet_pt_end; i_jet++)
	{
		for (int i_cent = 0; i_cent < 6; i_cent++)
		{
			...
			for (int i_trk = 0; i_trk < N_trkpt; i_trk++)
			{
				...
				h_ChPS_ratio_data_PbPb_data_pp_dR.at(i_trk).at(i_cent).at(i_jet)->GetYaxis()->SetTitle("#frac{PbPb}{pp} _{(#frac{1}{Area N_{Jets}} #frac{dN}{dp_{T}})}");
				...
				h_ChPS_noUnf_ratio_data_PbPb_data_pp_dR.at(i_trk).at(i_cent).at(i_jet)->GetYaxis()->SetTitle("#frac{PbPb}{pp} _{(#frac{1}{Area N_{Jets}} #frac{dN}{dp_{T}})}");
			...
			for (int i_dR = 0; i_dR < N_dR; i_dR++)
			{
				...
				h_ChPS_ratio_data_PbPb_data_pp.at(i_dR).at(i_cent).at(i_jet)->GetYaxis()->SetTitle("#frac{PbPb}{pp} _{(#frac{1}{Area N_{Jets}} #frac{dN}{dp_{T}})}");
				...
				h_ChPS_noUnf_ratio_data_PbPb_data_pp.at(i_dR).at(i_cent).at(i_jet)->GetYaxis()->SetTitle("#frac{PbPb}{pp} _{(#frac{1}{Area N_{Jets}} #frac{dN}{dp_{T}})}");
	...
	//final ratio for noUnf
	{
		cout << "Doing noUnf ChPS plots in jet pT" << endl;
		TCanvas *c_ChPS_noUnf_ratio = new TCanvas("c_ChPS_noUnf_ratio","c_ChPS_noUnf_ratio",900,600);
		TLegend *legend_ChPS_noUnf_ratio = new TLegend(0.20, 0.43, 0.40, 0.60, "","brNDC");
		legend_ChPS_noUnf_ratio->SetTextFont(43);
		legend_ChPS_noUnf_ratio->SetBorderSize(0);
		legend_ChPS_noUnf_ratio->SetTextSize(8);

		for (int i_dR = 0; i_dR < N_dR; i_dR++)
		{
			string dr_label = Form("%1.2f < dR < %1.2f", dR_binning->GetBinLowEdge(i_dR+1), dR_binning->GetBinUpEdge(i_dR+1));

			c_ChPS_noUnf_ratio->cd();
			c_ChPS_noUnf_ratio->Clear();
			c_ChPS_noUnf_ratio->Divide(3,2);
			...
			for (int i_cent = 0; i_cent < 6; i_cent++)
			{
				...
				for (int i_jet = jet_pt_start; i_jet < jet_pt_end; i_jet++)
				{
					...
					SetHStyle_smallify(h_ChPS_noUnf_ratio_data_PbPb_data_pp.at(i_dR).at(i_cent).at(i_jet), jet_itr, 1);
					if (i_dR == 0 && first_pass_cent) legend_ChPS_noUnf_ratio->AddEntry(h_ChPS_noUnf_ratio_data_PbPb_data_pp.at(i_dR).at(i_cent).at(i_jet),jet_label.c_str(),"lp");

					h_ChPS_noUnf_ratio_data_PbPb_data_pp.at(i_dR).at(i_cent).at(i_jet)->GetXaxis()->SetRangeUser(trk_pt_lo, trk_pt_hi);
					h_ChPS_noUnf_ratio_data_PbPb_data_pp.at(i_dR).at(i_cent).at(i_jet)->GetYaxis()->SetRangeUser(ratio_lo, ratio_hi);
					h_ChPS_noUnf_ratio_data_PbPb_data_pp.at(i_dR).at(i_cent).at(i_jet)->GetYaxis()->SetNdivisions(504);

					c_ChPS_noUnf_ratio->cd(i_cent+1);
					if (i_jet == jet_pt_start) h_ChPS_noUnf_ratio_data_PbPb_data_pp.at(i_dR).at(i_cent).at(i_jet)->Draw("");
					else h_ChPS_noUnf_ratio_data_PbPb_data_pp.at(i_dR).at(i_cent).at(i_jet)->Draw("same");
			...
			if (i_dR == 0) pdf_label = "(";
			if (i_dR == N_dR-1) pdf_label = ")";
			c_ChPS_noUnf_ratio->Print(Form("ChPS_noUnf_ratio.pdf%s", pdf_label.c_str()), Form("Title:dR%i", i_dR));
...

	{
		...
		cout << "Doing noUnf ChPS ratio (PbPb/pp) plots in dR" << endl;
		...
		for (int i_jet = jet_pt_start; i_jet < jet_pt_end; i_jet++)
		{
			...
			for (int i_cent = 0; i_cent < 6; i_cent++)
			{
				...
				for (int i_trk = 0; i_trk < N_trkpt; i_trk++)
				{
					...
					string trk_label = Form("%1.2f < p_{T}^{Trk} < %1.2f", trkpT_binning->GetBinLowEdge(i_trk+1), trkpT_binning->GetBinUpEdge(i_trk+1));
					SetHStyle_smallify(h_ChPS_noUnf_ratio_data_PbPb_data_pp_dR.at(i_trk).at(i_cent).at(i_jet), trk_itr, 1);
					if (jet_itr == 0 && first_pass_cent) legend_ChPS_dR->AddEntry(h_ChPS_noUnf_ratio_data_PbPb_data_pp_dR.at(i_trk).at(i_cent).at(i_jet),trk_label.c_str(),"lp");

					h_ChPS_noUnf_ratio_data_PbPb_data_pp_dR.at(i_trk).at(i_cent).at(i_jet)->GetXaxis()->SetRangeUser(0, 1.2);
					h_ChPS_noUnf_ratio_data_PbPb_data_pp_dR.at(i_trk).at(i_cent).at(i_jet)->GetYaxis()->SetRangeUser(ratio_lo, ratio_hi);
					h_ChPS_noUnf_ratio_data_PbPb_data_pp_dR.at(i_trk).at(i_cent).at(i_jet)->GetYaxis()->SetNdivisions(504);


					if (trk_itr == 0) h_ChPS_noUnf_ratio_data_PbPb_data_pp_dR.at(i_trk).at(i_cent).at(i_jet)->Draw("");
					else h_ChPS_noUnf_ratio_data_PbPb_data_pp_dR.at(i_trk).at(i_cent).at(i_jet)->Draw("same");
			...
			if (i_jet == jet_pt_start) pdf_label = "(";
			if (i_jet == jet_pt_end-1) pdf_label = ")";
			c_ChPS_dR->Print(Form("ChPS_noUnf_ratio_dR.pdf%s", pdf_label.c_str()), Form("Title:jetpt%i", i_jet));
			//
			jet_itr++;
