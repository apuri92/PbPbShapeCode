...
void draw_ChPS(string config_file = "ff_config.cfg")
{
		...
		for (int i_jet = jet_pt_start; i_jet < jet_pt_end; i_jet++)
		{
			...
			for (int i_dR = 0; i_dR < N_dR; i_dR++)
			{
				...
				for (int i_trk = 0; i_trk < N_trkpt; i_trk++)
				{
					...
					h_ChPS_raw_truthjet_indR.at(i_trk).at(i_cent).at(i_jet)->GetYaxis()->SetRangeUser(1E-5, 1E3);
					...
					h_ChPS_raw_truthjet_subtr_indR.at(i_trk).at(i_cent).at(i_jet)->GetYaxis()->SetNdivisions(504);
		...
		for (int i_dR = 0; i_dR < N_dR; i_dR++)
		{
			...
			for (int i_jet = jet_pt_start; i_jet < jet_pt_end; i_jet++)
			{
				...
				for (int i_cent = 0; i_cent < n_cent_cuts; i_cent++)
				{
					...
					SetHStyle_smallify(h_ChPS_UE.at(i_dR).at(i_cent).at(i_jet),2, doSmall);
					SetHStyle_smallify(h_ChPS_raw_subtr.at(i_dR).at(i_cent).at(i_jet),3, doSmall);
					SetHStyle_smallify(h_ChPS_raw_subtr_unf.at(i_dR).at(i_cent).at(i_jet),4, doSmall);
					SetHStyle_smallify(h_ChPS_raw_subtr_unf_bbb.at(i_dR).at(i_cent).at(i_jet),5, doSmall);

					SetHStyle_smallify(h_ChPS_ratio_subtr_raw.at(i_dR).at(i_cent).at(i_jet),6, doSmall);
					SetHStyle_smallify(h_ChPS_ratio_unf_subtr.at(i_dR).at(i_cent).at(i_jet),7, doSmall);
					SetHStyle_smallify(h_ChPS_ratio_closure.at(i_dR).at(i_cent).at(i_jet),8, doSmall);
	...
	//Draw Final ChPS plots
	{
		...
		for (int i_dR = 0; i_dR < N_dR; i_dR++)
		{
			...
			for (int i_cent = 0; i_cent < n_cent_cuts; i_cent++)
			{
				...
				if (isMC) setup_canvas(c_ChPS_final, dataset_type, i_cent);
				...
				for (int i_jet = jet_pt_start; i_jet < jet_pt_end; i_jet++)
				{
					...
					if (isMC && dataset_type == "pp") c_ChPS_final->cd(1);
					if (isMC && dataset_type == "PbPb") c_ChPS_final->cd(i_cent+1)->cd(1);
					if (!isMC && dataset_type == "pp") c_ChPS_final->cd();
					if (!isMC && dataset_type == "PbPb") c_ChPS_final->cd(i_cent+1);
	...
	//Draw Final ChPS plots in jet
	{
		...
		for (int i_cent = 0; i_cent < n_cent_cuts; i_cent++)
		{
			...
			if (isMC) setup_canvas(c_ChPS_final_inJet, dataset_type, i_cent);
			...
			for (int i_jet = jet_pt_start; i_jet < jet_pt_end; i_jet++)
			{
				...
				if (isMC && dataset_type == "pp") c_ChPS_final_inJet->cd(1);
				if (isMC && dataset_type == "PbPb") c_ChPS_final_inJet->cd(i_cent+1)->cd(1);
				if (!isMC && dataset_type == "pp") c_ChPS_final_inJet->cd();
				if (!isMC && dataset_type == "PbPb") c_ChPS_final_inJet->cd(i_cent+1);
...
//					h_ChPS_UE.at(i_dR).at(i_cent).at(i_jet)->GetYaxis()->SetRangeUser(0.2*min,1.05*max);
	...
	//draw as function of dR
	{
		...
		for (int i_jet = jet_pt_start; i_jet < jet_pt_end; i_jet++)
		{
			...
			for (int i_cent = 0; i_cent < n_cent_cuts; i_cent++)
			{
				...
				if (isMC) setup_canvas(c_ChPS_dR, dataset_type, i_cent);
				...
				for (int i_trk = trk_pt_start; i_trk < trk_pt_end; i_trk++)
				{
					...
					if (isMC && dataset_type == "pp") c_ChPS_dR->cd(1);
					if (isMC && dataset_type == "PbPb") c_ChPS_dR->cd(i_cent+1)->cd(1);
					if (!isMC && dataset_type == "pp") c_ChPS_dR->cd();
					if (!isMC && dataset_type == "PbPb") c_ChPS_dR->cd(i_cent+1);
	...
	{
		cout << "Doing Final ChPS plots (as function of r, for trk pT) for tracks in truth jets" << endl;
		...
		for (int i_jet = jet_pt_start; i_jet < jet_pt_end; i_jet++)
		{
			...
			for (int i_cent = 0; i_cent < n_cent_cuts; i_cent++)
			{
				...
				if (isMC) setup_canvas(c_ChPS_truthjet_dR, dataset_type, i_cent);
 				...
 				for (int i_trk = trk_pt_start; i_trk < trk_pt_end; i_trk++)
				{
					...
					if (isMC && dataset_type == "pp") c_ChPS_truthjet_dR->cd(1);
					if (isMC && dataset_type == "PbPb") c_ChPS_truthjet_dR->cd(i_cent+1)->cd(1);
					if (!isMC && dataset_type == "pp") c_ChPS_truthjet_dR->cd();
					if (!isMC && dataset_type == "PbPb") c_ChPS_truthjet_dR->cd(i_cent+1);
			...
			if (i_jet == jet_pt_start) pdf_label = "(";
			if (i_jet == jet_pt_end-1) pdf_label = ")";
			c_ChPS_truthjet_dR->Print(Form("output_pdf/%s/ChPS_final_dR_truthjet_%s_%s.pdf%s", dataset_type.c_str(), dataset_type.c_str(), did.c_str(), pdf_label.c_str()), Form("Title:jetpt%i", i_jet));
		...
		for (int i_jet = jet_pt_start; i_jet < jet_pt_end; i_jet++)
		{
 			...
 			for (int i_trk = trk_pt_start; i_trk < trk_pt_end; i_trk++)
			{
				...
				for (int i_cent = 0; i_cent < n_cent_cuts; i_cent++)
				{
					...
					if (isMC) setup_canvas(c_evol_dR, dataset_type, i_cent);

					if (isMC && dataset_type == "pp") c_evol_dR->cd(1);
					if (isMC && dataset_type == "PbPb") c_evol_dR->cd(i_cent+1)->cd(1);
					if (!isMC && dataset_type == "pp") c_evol_dR->cd();
					if (!isMC && dataset_type == "PbPb") c_evol_dR->cd(i_cent+1);
		...
		for (int i_jet = jet_pt_start; i_jet < jet_pt_end; i_jet++)
		{
			...
			for (int i_trk = trk_pt_start; i_trk < trk_pt_end; i_trk++)
			{
				...
				for (int i_cent = 0; i_cent < n_cent_cuts; i_cent++)
				{
					...
					SetHStyle_smallify( h_ChPS_truth_indR.at(i_trk).at(i_cent).at(i_jet),0, doSmall);
					SetHStyle_smallify( h_ChPS_raw_truthjet_indR.at(i_trk).at(i_cent).at(i_jet),1, doSmall);
					SetHStyle_smallify( h_ChPS_UE_truthjet_indR.at(i_trk).at(i_cent).at(i_jet),2, doSmall);
					SetHStyle_smallify( h_ChPS_raw_truthjet_subtr_indR.at(i_trk).at(i_cent).at(i_jet),3, doSmall);
					SetHStyle_smallify( h_ChPS_raw_truthjet_subtr_bbb_indR.at(i_trk).at(i_cent).at(i_jet),4, doSmall);
					SetHStyle_smallify(h_ChPS_ratio_closure_truthjet_indR.at(i_trk).at(i_cent).at(i_jet),4, doSmall);
					...
					if (i_jet == jet_pt_start && trk_itr == 0 && first_pass)
					{
						...
						if (dataset_type == "PbPb")
						{
							legend_evol_truthjet_dR->AddEntry(h_ChPS_UE_truthjet_indR.at(i_trk).at(i_cent).at(i_jet),"UE_{TJ}","lp");
...
//					}
					...
					if (!isMC && dataset_type == "pp") c_evol_truthjet_dR->cd();
					if (!isMC && dataset_type == "PbPb") c_evol_truthjet_dR->cd(i_cent+1);
					...
					if (isMC)
					{
						...
						if (dataset_type == "PbPb") c_evol_truthjet_dR->cd(i_cent+1)->cd(2);
						if (isMC) h_ChPS_ratio_closure_truthjet_indR.at(i_trk).at(i_cent).at(i_jet)->Draw();
				...
				if (i_trk == trk_pt_start && i_jet == jet_pt_start) pdf_label = "(";
				if (i_trk == trk_pt_end-1 && i_jet == jet_pt_end-1) pdf_label = ")";
				c_evol_truthjet_dR->Print(Form("output_pdf/%s/evol_dR_truthjet_%s_%s.pdf%s", dataset_type.c_str(), dataset_type.c_str(), did.c_str(), pdf_label.c_str()), Form("Title:trk%i_jetpt%i", i_trk, i_jet));
	...
	{
		cout << "Doing Final ChPS plots (as function of trk pT, for jet pT) for tracks in truth jet" << endl;
		...
		for (int i_dR = 0; i_dR < N_dR; i_dR++)
		{
			...
			for (int i_cent = 0; i_cent < n_cent_cuts; i_cent++)
			{
				...
				if (isMC) setup_canvas(c_ChPS_truthjet_final, dataset_type, i_cent);
				...
				for (int i_jet = jet_pt_start; i_jet < jet_pt_end; i_jet++)
				{
					...
					if (isMC && dataset_type == "pp") c_ChPS_truthjet_final->cd(1);
					if (isMC && dataset_type == "PbPb") c_ChPS_truthjet_final->cd(i_cent+1)->cd(1);
					if (!isMC && dataset_type == "pp") c_ChPS_truthjet_final->cd();
					if (!isMC && dataset_type == "PbPb") c_ChPS_truthjet_final->cd(i_cent+1);
			...
			if (i_dR == 0) pdf_label = "(";
			if (i_dR == N_dR-1) pdf_label = ")";
			c_ChPS_truthjet_final->Print(Form("output_pdf/%s/ChPS_final_truthjet_%s_%s.pdf%s", dataset_type.c_str(), dataset_type.c_str(), did.c_str(), pdf_label.c_str()), Form("Title:dR%i", i_dR));
