		...
		for (int i_dR = 0; i_dR < N_dR; i_dR++)
		{
			...
			for (int i_cent = 0; i_cent < n_cent_cuts; i_cent++)
			{
				...
				for (int i_jet = jet_pt_start; i_jet < jet_pt_end; i_jet++)
				{
					...
					if (dataset_type == "PbPb") SetHStyle_smallify(h_ChPS_UE.at(i_dR).at(i_cent).at(i_jet), jet_itr, doSmall);
		...
		for (int i_jet = jet_pt_start; i_jet < jet_pt_end; i_jet++)
		{
			...
			for (int i_trk = trk_pt_start; i_trk < trk_pt_end; i_trk++)
			{
				...
				for (int i_cent = 0; i_cent < n_cent_cuts; i_cent++)
				{
					...
					if (dataset_type == "PbPb") SetHStyle_smallify( h_ChPS_UE_truthjet_indR.at(i_trk).at(i_cent).at(i_jet),2, doSmall);
