...
void combine_eff_dr_jety_reduced(vector<TFile*>& theFiles, vector<double> w)
{
...
//	vector<vector<vector<vector<TGraphAsymmErrors*>>>> g_efficiency(n_cent_cuts, vector<vector<vector<TGraphAsymmErrors*>>> (n_dR, vector<vector<TGraphAsymmErrors*>> (n_jetpt_bins, vector<TGraphAsymmErrors*> (n_jety_bins))));
	...
	int start_jetpt = 7, end_jetpt = 11, start_dr = 0, end_dr = 11;
	...
	for (int i_cent_cuts = 0; i_cent_cuts < 6; i_cent_cuts++)
	{
		...
		for (int i_dR = start_dr; i_dR < end_dr; i_dR++)
		{
			double dr_lo = dR_binning->GetBinLowEdge(i_dR+1);
			double dr_hi = dR_binning->GetBinUpEdge(i_dR+1);

			if (dr_lo > 0.79) continue;

			for (int i_jetpt_cuts = start_jetpt; i_jetpt_cuts < end_jetpt; i_jetpt_cuts++)
			{
...
//					cout << full_label << endl;
					...
					TH1* h_efficiency;
					TGraphAsymmErrors* g_efficiency;
					for (int i_files = 0; i_files < nFiles; i_files++)
					{
						...
						if (i_files == 0)
						{
							...
							h_efficiency = (TH1*)h_matched_1d->Clone(name.c_str());
							h_efficiency->Reset();
						}
						...
						if (1)
						{
							...
							cout << i_jety_cuts << " " <<  i_jetpt_cuts << " " <<  i_cent_cuts << " " << i_dR << " " <<  i_files << endl;
							if (i_jety_cuts == 0 && i_jetpt_cuts == start_jetpt && i_cent_cuts == 0 && i_dR == start_dr && i_files == 0) name = "(";
							else if (i_jety_cuts == n_jety_bins - 1 && i_jetpt_cuts == end_jetpt-1 && i_cent_cuts == 5 && i_dR == end_dr-1 && i_files == nFiles - 1) name = ")";
							else name = "";
							cout << name << endl;
					...
					for (int i_bin = 0; i_bin < h_efficiency->GetNbinsX(); i_bin++)
					{
						...
						h_efficiency->SetBinContent(i_bin+1, efficiency);
						h_efficiency->SetBinError(i_bin+1, efficiency_lo);
					}
...
//					output_file->cd();
//
//					g_efficiency = new TGraphAsymmErrors(h_efficiency);
//					g_efficiency->SetTitle(full_label.c_str());
//
//					name = Form("graph_eff_cent%i_dr%i_jety%i_jetpt%i",i_cent_cuts, i_dR, i_jety_cuts, i_jetpt_cuts);
//					g_efficiency->SetName(name.c_str());
//					g_efficiency->Write(name.c_str());
//
//					h_efficiency->SetTitle(full_label.c_str());
//					name = Form("hist_eff_cent%i_dr%i_jety%i_jetpt%i",i_cent_cuts, i_dR, i_jety_cuts, i_jetpt_cuts);
//					h_efficiency->SetName(name.c_str());
//					h_efficiency->Write(name.c_str());
