...
//	h_tmp_data->Scale(1./h_tmp_data->Integral());
	...
	vector<vector<TH2*>> h_MB_rN_2D = vector<vector<TH2*>> (N_dR, vector<TH2*> (n_cent_cuts));
	...
	vector<TH2*> h_MB_rN_injet_2D = vector<TH2*> (n_cent_cuts);
	...
	for (int i_run = run_start; i_run <= run_end; i_run++)
	{
		...
		for (int i_cent = 0; i_cent < 6; i_cent++)
		{
			...
			for (int i_dR = 0; i_dR < N_dR; i_dR++)
			{
				...
				h_MB_2D[i_dR][i_cent] = (TH2*)input_file_data->Get(name.c_str())->Clone(Form("%s_run%i",name.c_str(), i_run));
				...
				h_MB_rN_2D[i_dR][i_cent] = (TH2*)h_MB_rN_3D->Project3D("zy");
				h_MB_rN_2D[i_dR][i_cent]->SetName(Form("%s_run%i",name.c_str(), i_run));
				...
				for (int i_jet = jet_pt_start; i_jet <= jet_pt_end; i_jet++)
				{
					...
					for (int i_trk = trk_pt_start; i_trk <= trk_pt_end; i_trk++)
					{
						...
						if (n_jets_MB != 0)
						{
							double updated_UE_MB = h_MB_2D[i_dR][i_cent]->GetBinContent(i_trk, i_jet) / n_jets_MB;
							double updated_UE_MB_err = h_MB_2D[i_dR][i_cent]->GetBinError(i_trk, i_jet) / n_jets_MB;
						...
						if (n_jets_MB_rN != 0)
						{
							double updated_UE_MB_rN = h_MB_rN_2D[i_dR][i_cent]->GetBinContent(i_trk, i_jet) / n_jets_MB_rN;
							double updated_UE_MB_rN_err = h_MB_rN_2D[i_dR][i_cent]->GetBinError(i_trk, i_jet) / n_jets_MB_rN;
							h_MB_rN_2D[i_dR][i_cent]->SetBinContent(i_trk, i_jet, updated_UE_MB_rN);
							h_MB_rN_2D[i_dR][i_cent]->SetBinError(i_trk, i_jet, updated_UE_MB_rN_err);
						}
						else h_MB_rN_2D[i_dR][i_cent]->Reset();
				...
				for (int i_jet = jet_pt_start-1; i_jet < jet_pt_end; i_jet++)
				{
					...
					h_MB_rN_1D[i_jet][i_dR][i_cent] = (TH1*)h_MB_rN_2D[i_dR][i_cent]->ProjectionX(Form("MB_rN_jet%i_dr%i_cent%i", i_jet, i_dR, i_cent), i_jet+1, i_jet+1);
