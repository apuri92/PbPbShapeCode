...
//				h_combined->Draw("same hist text");
			...
			c->Print(Form("misc_plots/weightedRuns.pdf%s",pdf_label.c_str()),Form("Title: jet%i_trk%i", i_jet, i_trk));
			c1->Print(Form("misc_plots/weightedRuns_v_Nominal.pdf%s",pdf_label.c_str()),Form("Title: jet%i_trk%i", i_jet, i_trk));
			c2->Print(Form("misc_plots/Nominal_full_v_reducedMaps.pdf%s",pdf_label.c_str()),Form("Title: jet%i_trk%i", i_jet, i_trk));
