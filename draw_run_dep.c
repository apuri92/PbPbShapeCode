...
void draw_run_dep(int config = 37)
{
	...
	vector<vector<vector<vector<TF1*>>>> func_UE_run_dep = vector<vector<vector<vector<TF1*>>>> (N_jetpt, vector<vector<vector<TF1*>>> (N_trkpt, vector<vector<TF1*>> (N_dR, vector<TF1*> (6)))); //histos from run_dep.c are such that h_XXX_run(i) where i is bin number (so starting at 1)

	vector<vector<TH1*>> h_jet_run_dep_data = vector<vector<TH1*>> (N_jetpt, vector<TH1*> (6)); //histos from run_dep.c are such that h_XXX_run(i) where i is bin number (so starting at 1)
	...
	for (int i_jet = jet_pt_start-1; i_jet < jet_pt_end; i_jet++)
	{
		for (int i_cent = 0; i_cent < 6; i_cent++)
		{
			...
			for (int i_trk = trk_pt_start-1; i_trk < trk_pt_end; i_trk++)
			{
				...
				{
					h_UE_run_dep[i_jet][i_trk][i_dR][i_cent] = (TH1*)h_eventPercentage_mc->Clone(Form("UE_run_dep_jet%i_trk%i_dR%i_cent%i", i_jet, i_trk, i_dR, i_cent));
